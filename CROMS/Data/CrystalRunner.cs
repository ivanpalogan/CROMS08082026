...
namespace CROMS.Data
{
    ...
    internal static class CrystalRunner
    {
                                ...
                                FormDefinition layout = null)
        {
            ...
            // A report generated by CROMS.ReportGen binds to a purpose-built print table
            // (every box already resolved to its text, from the form's own print map); a
            // report authored by hand in the Crystal designer binds to the flat report view.
            bool generated = false;
            foreach (CrystalDecisions.CrystalReports.Engine.Table t in report.Database.Tables)
                if (string.Equals(t.Name, CertificateReport.PrintTableName,
                                  StringComparison.OrdinalIgnoreCase)) generated = true;

            DataTable bound = generated && data.Rows.Count > 0
                ? CertificateReport.BuildPrintTable(layout ?? def, data.Rows[0], stamp)
                : WithBranding(data, logo, stamp);
